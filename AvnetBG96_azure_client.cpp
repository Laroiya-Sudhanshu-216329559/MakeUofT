...
static const char* connectionString = "HostName=DividedbyZer0.azure-devices.net;DeviceId=telusboard;SharedAccessKey=c6VVh29ywpTTgBxmTOQtRiQ0KuY8q4p9fKOMino+aBc=";

static const char* deviceId         = "telusboard";; /*must match the one on connectionString*/
...
int main(void)
{
  ...
  acc_gyro = mems_expansion_board->GetGyroscope();
    ...
    mems_init();
    LED_thread.start(LED_task); //sending to board
    motor_task();
    azure_client_thread.start(azure_task); //sending to board
...
//-------------------------------------------------------------------message to send to the board--------------------------------------------------------------
    printf("Receiving message: '%s'\r\n", temp);
    if( !strcmp(temp,"light-blink") ) {
        SET_LED(RED,LED_BLINK);
        printf("start blinking\n");
        }
    if( !strcmp(temp,"light-on") ) {
        SET_LED(RED,LED_ON);
        printf("turn on\n");
        }
    if( !strcmp(temp,"light-off") ) {
        SET_LED(RED,LED_OFF);
        printf("turn off\n");
        }
